...
void Bubble_sort(
	...
	int j,max = n%chunckSize==0 ? n/chunckSize : n/chunckSize+1;
	omp_lock_t *lock = (omp_lock_t*) malloc(sizeof(omp_lock_t) * max);
	...
	#pragma omp parallel shared(n)
	{
		while ( n>1 ) {
			...
			for (block=0; block<max; block++) {		
				...
				if (end <= currSize) {
					omp_set_lock(&lock[block+1]);
					if (a[end-1] > a[end]) {
						temp = a[end-1];
						a[end-1] = a[end];
						a[end] = temp;
					}
					omp_unset_lock(&lock[block]);
				}
				else {
					omp_unset_lock(&lock[block]);
					break;
