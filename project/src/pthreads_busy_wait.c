...
inline void set_lock(int i, int me) {
	while (locks[i]!=me) {
		fflush(stdout);
	}		
}

inline void unset_lock(int i) {
...
void Bubble_sort(
	...
	GET_TIME(start);
	long i;
	max = n%chunckSize==0 ? n/chunckSize : n/chunckSize+1;
	
	//create locks
	locks = (int*) calloc(max ,sizeof(int));
	...
	for(i=0; i<nthreads-1; i++)
		pthread_create(threads+i, NULL, sort, (void*)i/*(void*)arguments*/);

	sort((void*)nthreads-1 /*(void*) arguments*/);
