 ...
 * In/out args:	a
 */
inline void set_lock( int *locks, int i) {
	int me=omp_get_thread_num();

	//printf("max=%d; locks=[%d,%d]; locks[%d] == %d?\n",nthreads,locks[0],locks[1],i,me );
	while (locks[i]!=me) {
		#pragma omp flush
	}
	//printf("\tThread %d locked block %d\n",me,i );
		
}

inline void unset_lock( int *locks, int i) {
	//printf("\tThread %d unlocked block %d\n",omp_get_thread_num(),i );
	locks[i] = (locks[i]+1) % nthreads;
}
...
void Bubble_sort(
	...
	int max = n%chunckSize==0 ? n/chunckSize : n/chunckSize+1;
	int *locks = (int*) calloc(max ,sizeof(int));
	//Print_list(a, 6, "initial");
	#pragma omp parallel shared(n,a)
	{
		while ( n>1 ) {
			...
			for (block=0; block<max; block++) {		
				...
				if (end <= currSize) {
	            	set_lock( locks, block+1 );
	            	//printf("%d > %d?\t",a[end-1], a[end] );
					if (a[end-1] > a[end]) {
						temp = a[end-1];
						a[end-1] = a[end];
						a[end] = temp;
					...
					//else printf("\n");
					unset_lock( locks, block );
				}
				else {
					//char *tmp=malloc(64);
					//sprintf(tmp,"thread %d, block %d, size:%d",omp_get_thread_num(),block,currSize);
					//Print_list(a, 6, tmp);
					//#pragma omp flush
					unset_lock( locks, block );
					break;
